...
export async function GET(req: NextRequest) {
  try {
    ...
    if (tag) { sql += " AND EXISTS (SELECT 1 FROM book_tags bt WHERE bt.book_id = b.id AND bt.tag = ?)"; args.push(tag); }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      pub_year, publisher, pub_place, series, contributors, tags, related,
    } = await req.json();
    ...
    if (Array.isArray(tags)) {
      for (const tag of tags) {
        if (tag?.trim()) {
          await db.execute({
            sql: "INSERT OR IGNORE INTO book_tags (book_id, tag) VALUES (?, ?)",
            args: [id, tag.trim().toLowerCase()],
          });
        }
      }
    }

    // Insert related books (bidirectional)
    if (Array.isArray(related)) {
      for (const relId of related) {
        if (relId && relId !== id) {
          await db.execute({
            sql: "INSERT OR IGNORE INTO book_relations (book_id, related_book_id) VALUES (?, ?)",
            args: [id, relId],
          });
          await db.execute({
            sql: "INSERT OR IGNORE INTO book_relations (book_id, related_book_id) VALUES (?, ?)",
    ...
    const bookContributors = await db.execute({ sql: "SELECT * FROM book_contributors WHERE book_id = ? ORDER BY position", args: [id] });
    const bookTags = await db.execute({ sql: "SELECT tag FROM book_tags WHERE book_id = ?", args: [id] });
    return NextResponse.json({ book: { ...book.rows[0], contributors: bookContributors.rows, tags: bookTags.rows.map((r: any) => r.tag) } }, { status: 201 });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : "Error" }, { status: 500 });
  }
}
